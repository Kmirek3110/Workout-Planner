...
class Workout:
    """
    Klasa odpowiadające klasie modelowej django pomocna w generowaniu 
    planów.
    """
...
def generate_plan(workouts, type, mandatory_exercises, time_per_workout=90):
    ...
    if split != "PowerliftingSplit":
        ...
        for i, workout in enumerate(plan):
            current_exercises = mandatory_exercises.filter(type__in = workout.muscle_split.keys())
    ...
    if split == "FullBodyWorkout":
        ...
        for workout in plan:
            while workout.time >= sets * rest_time:
                ...
                types = [k for k, v in workout.muscle_split.items() if v==minval]
        ...
        for i, workout in enumerate(plan):
            current_exercises = hard_exercises.filter(type__in = workout.muscle_split.keys())
        ...
        print(list(hard_exercises))
        hard_exercises = random.sample(list(hard_exercises), k = 4)
        for i, exercise in enumerate(hard_exercises):
            plan[i].muscle_split[exercise.type] += 1
            plan[i].time -= (sets * rest_time)+1
            plan[i].exercises.append(exercise)
            plan_exercises.append(exercise)
